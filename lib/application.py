...
import mongotest.conf.config as cfg
from mongotest.lib.powlib import merge_two_dicts
from mongotest.database.sqldblib import Base, Session, engine
from mongotest.conf.config import myapp
from mongotest.conf.config import routes
...
def route(route, dispatch=[], params=[], schema=[], pos=-1):
...
class Application(tornado.web.Application):
    ...
    def import_all_handlers(self):
        ...
        import importlib
        for m in mods:
            #print("importing: " + 'mongotest.handlers.' + m)    
            try:
                mod = importlib.import_module('mongotest.handlers.' + m)
            except:
                pass
            #print(dir(mod))
        ...
        def decorator(cls):
            ...
            for name, method in cls.__dict__.items():
                if hasattr(method, "routed"):
                    ...
                    schema = method.route.get("schema")
