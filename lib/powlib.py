...
from mongotest.conf.config import myapp, server_settings
...
class powDecNew():
    ...
    def has_many(self, child_as_str, backref=False):
        ...
        def decorator(parent_class):
            ...
            import sys
            if "mongotest.models.sql." + child_module_name in sys.modules.keys():
                #print(dir(sys.modules["mongotest.models.sql." + child_module_name]))
                child_klass = getattr(sys.modules["mongotest.models.sql." + child_module_name], child_class_name)
            else:
                import importlib
                mod = importlib.import_module('mongotest.models.sql.' + child_module_name)
                #mod = __import__('mongotest.models.sql.'+rel_module_name, fromlist=[rel_class_name])
    ...
    def belongs_to(self, parent_as_str):
        ...
        def decorator(child_class):
            ...
            parent_class = None
            import sys
            if ("mongotest.models.sql."+parent_module_name) in sys.modules.keys():
                parent_class = getattr(sys.modules["mongotest.models.sql."+parent_module_name], parent_class_name)
            else:
                import importlib
                mod = importlib.import_module('mongotest.models.sql.' + parent_module_name)
                #mod = __import__('mongotest.models.sql.'+rel_module_name, fromlist=[rel_class_name])
    ...
    def one_to_one(self, child_as_str):
        ...
        def decorator(parent):
            ...
            mod = __import__('mongotest.models.sql.'+child_module_name, fromlist=[child_class_name])
    ...
    def many_to_many(self, children):
        ...
        # rel_as_str is the plueral name of the child class (adresses, comments)
        # klass below is the real class instance of the child
        from mongotest.database.sqldblib import Base
        def decorator(parent_class):
            parent_name = parent_class.__name__.lower()
            parent_class_name = parent_class.__name__
