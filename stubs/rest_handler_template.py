...
class {{handler_class_name}}(PowHandler):
    ...
    @route(r'/{{handler_name}}/<uuid:id>', dispatch=["get"] )
    def show(self, id: UUID) -> model.schema:
        """ show model with uuid = id """
        m=Model()
        res=m.find_by_id(id)
        self.success(message="{{handler_name}} show", data=res)
    
    @route(r'/{{handler_name}}', dispatch=["get"])
    def list(self) -> list[model.schema]:
        m=Model()
        res = m.get_all()  
        self.success(message="{{handler_name}}, index", data=res)         
    
    @route(r'/{{handler_name}}/page/<int:page>', dispatch=["get"])
    def page(self, page:int = 0) -> list[model.schema]:
        m=Model()
        res=m.page(page=int(page), page_size=myapp["page_size"])
        self.success(message="{{handler_name}} page: #" +str(page), data=res )  
        
    @tornado.web.authenticated
    @route(r'/{{handler_name}}/<uuid:id>/edit', dispatch=["get"])
    ...
    @tornado.web.authenticated
    @route(r'/{{handler_name}}/new', dispatch=["get"])
    def new(self):
        m=Model()
        self.success(message="{{handler_name}}, new",data=m)

    @tornado.web.authenticated
    @route(r'/{{handler_name}}/create', dispatch=["post"])
    def create(self):
        try:
    ...
    @tornado.web.authenticated
    @route(r'/{{handler_name}}/<uuid:id>', dispatch=["put"])
    ...
    @tornado.web.authenticated
    @route(r'/{{handler_name}}/<uuid:identifier>', dispatch=["delete"])
    ...
    @route(r'/{{handler_name}}/search', dispatch=["get"])
    def search(self):
        m=Model()
