...
def test_sql_generate_model(modelname):
    ...
    import mongotest.generate_model as gm
    import uuid
    import os.path
    ret = gm.generate_model(modelname, "sql", appname="mongotest")
...
def test_sql_model_type():
    ...
    from mongotest.models.sql.pow_test_model import PowTestModel
    m = PowTestModel()
...
def test_sql_dbsetup():
    ...
    import mongotest.init_sqldb_environment
    import os
    os.chdir("..")
    r = mongotest.init_sqldb_environment.init_migrations()
    assert r == True
...
def test_sql_migration():
    ...
    import mongotest.generate_migration
    import os
    os.chdir("..")
    script = mongotest.generate_migration.generate_migration(message="pow_test")
...
def test_sql_dbupdate():
    ...
    import mongotest.update_db
    import os, time
    ret = None
    os.chdir("..")
    time.sleep(1)
    try:
        ret = mongotest.update_db.migrate("up")
